...
# Script para iniciar cliente, servidor y túneles Cloudflare automáticamente

echo "========================================"
echo "  Que Comemos - Inicio de Desarrollo"
echo "========================================"
echo ""
echo "Este script iniciará:"
echo "  1. Cliente (puerto 5173)"
echo "  2. Servidor Backend (puerto 3001)"
echo "  3. Túnel Cloudflare para el servidor (URL pública)"
echo "  4. Túnel Cloudflare para el cliente (URL pública)"
echo ""
...
cleanup() {
    ...
    if [ -n "$SERVER_TUNNEL_PID" ]; then
        kill $SERVER_TUNNEL_PID 2>/dev/null
    fi
    if [ -n "$CLIENT_TUNNEL_PID" ]; then
        kill $CLIENT_TUNNEL_PID 2>/dev/null
    fi
    exit 0
}
...
echo -e "${YELLOW}Verificando dependencias del cliente...${NC}"
...
echo -e "${YELLOW}Verificando dependencias del servidor...${NC}"
...
cd client
npm run dev > ../server/client.log 2>&1 &
CLIENT_PID=$!
cd ..
...
if ! kill -0 $CLIENT_PID 2>/dev/null; then
    echo -e "${RED}Error: El cliente no pudo iniciarse. Revisa server/client.log${NC}"
    exit 1
fi
...
if ! kill -0 $SERVER_PID 2>/dev/null; then
    echo -e "${RED}Error: El servidor no pudo iniciarse. Revisa server/server.log${NC}"
    kill $CLIENT_PID 2>/dev/null
    exit 1
fi
...
update_client_env() {
    ...
    if grep -qE "^[[:space:]]*VITE_API_URL[[:space:]]*=" "$client_env"; then
        ...
        echo "" >> "$client_env"
        echo "# URL del servidor backend (actualizado automáticamente por start.sh)" >> "$client_env"
...
if [ -n "$CLOUDFLARED_PATH" ]; then
    ...
    $CLOUDFLARED_PATH tunnel --url http://localhost:3001 > server/server-tunnel.log 2>&1 &
    SERVER_TUNNEL_PID=$!
    
    # Esperar y buscar la URL del túnel del servidor
    echo "Esperando a que el túnel del servidor se establezca..."
    SERVER_TUNNEL_URL=$(detect_tunnel_url "server/server-tunnel.log")
    ...
    if [ -n "$SERVER_TUNNEL_URL" ]; then
        ...
        echo "Revisa server/server-tunnel.log para ver la URL manualmente"
    fi
    ...
    if curl -s http://localhost:5173 > /dev/null 2>&1; then
        ...
        $CLOUDFLARED_PATH tunnel --url http://localhost:5173 > server/client-tunnel.log 2>&1 &
        CLIENT_TUNNEL_PID=$!
        
        # Esperar y buscar la URL del túnel del cliente
        echo "Esperando a que el túnel del cliente se establezca..."
        CLIENT_TUNNEL_URL=$(detect_tunnel_url "server/client-tunnel.log")
        ...
        if [ -n "$CLIENT_TUNNEL_URL" ]; then
            ...
            echo "Revisa server/client-tunnel.log para ver la URL manualmente"
        fi
    else
        echo -e "${YELLOW}Cliente no detectado en puerto 5173${NC}"
        echo "Esperando unos segundos más..."
        sleep 5
        if curl -s http://localhost:5173 > /dev/null 2>&1; then
            echo -e "${GREEN}✓ Cliente detectado, iniciando túnel...${NC}"
            $CLOUDFLARED_PATH tunnel --url http://localhost:5173 > server/client-tunnel.log 2>&1 &
            CLIENT_TUNNEL_PID=$!
            CLIENT_TUNNEL_URL=$(detect_tunnel_url "server/client-tunnel.log")
            if [ -n "$CLIENT_TUNNEL_URL" ]; then
                echo -e "${GREEN}✓ Túnel del cliente iniciado: $CLIENT_TUNNEL_URL${NC}"
            fi
        fi
    fi
fi
...
echo "Logs:"
echo "  - Cliente:        tail -f server/client.log"
echo "  - Servidor:       tail -f server/server.log"
if [ -n "$SERVER_TUNNEL_PID" ]; then
    echo "  - Túnel servidor: tail -f server/server-tunnel.log"
fi
if [ -n "$CLIENT_TUNNEL_PID" ]; then
    echo "  - Túnel cliente:  tail -f server/client-tunnel.log"
fi
echo ""
echo "Presiona Ctrl+C para detener todo"
echo "========================================"
