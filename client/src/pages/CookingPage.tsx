...
import { useShoppingList, useIngredients, updateIngredient, useWeek, markOneRecipeInstanceAsCompleted } from "../hooks/useStorage";
...
export default function CookingPage() {
  ...
  const { week } = useWeek();
  const [recipeAvailabilities, setRecipeAvailabilities] = useState<Map<string, RecipeAvailability>>(new Map());
  const [editingInventoryMeasures, setEditingInventoryMeasures] = useState<Map<string, Record<string, string>>>(new Map());
    ...
    const loadAvailabilities = async () => {
      ...
      for (const recipeList of availableRecipes) {
        try {
          ...
          console.error(`Error loading availability for recipe ${recipeList.recipeName}:`, error);
        }
      }
  ...
  useEffect(() => {
    ...
    const allRecipeNames = new Set(availableRecipes.map((list) => list.recipeName));
  ...
  const handleInventoryMeasureChange = (recipeName: string, ingredientName: string, newMeasure: string) => {
  ...
  const handleConfirmIngredientInventory = async (recipeName: string, ingredientName: string) => {
    const recipeMeasures = editingInventoryMeasures.get(recipeName);
    if (!recipeMeasures || !recipeMeasures[ingredientName]) return;

    const newMeasure = recipeMeasures[ingredientName];

    try {
      // Aplicar el cambio al inventario
      await updateIngredient(ingredientName, { measure: newMeasure });
      ...
      setEditingInventoryMeasures((prev) => {
        const newMap = new Map(prev);
        const recipeMeasures = newMap.get(recipeName) || {};
        delete recipeMeasures[ingredientName];
  ...
  const handleCancelIngredientEdit = (recipeName: string, ingredientName: string) => {
    setEditingInventoryMeasures((prev) => {
      const newMap = new Map(prev);
      const recipeMeasures = newMap.get(recipeName) || {};
      delete recipeMeasures[ingredientName];
      ...
      {availableRecipes.length === 0 ? (
        <Card>
          <CardContent>
            <Typography align="center" color="text.secondary" sx={{ py: 4 }}>
              {t("cooking.noRecipes") || "No hay recetas listas para cocinar. Compra todos los ingredientes faltantes primero."}
            </Typography>
          </CardContent>
        </Card>
      ) : (
        <Box>
          {availableRecipes.map((recipeList) => {
            const availability = recipeAvailabilities.get(recipeList.recipeName);
            const recipeMeasures = editingInventoryMeasures.get(recipeList.recipeName) || {};
                ...
                <CardContent>
                  ...
                  {expandedRecipes.has(recipeList.recipeName) && (
                    <>
                      ...
                      {availability && availability.availableIngredients.length > 0 && (
                        <Box sx={{ mb: 3 }}>
                          ...
                          <List dense>
                            {availability.availableIngredients.map((match, idx) => {
                              const originalMeasure = match.inventoryIngredient.measure || "";
                              const currentMeasure =
                                recipeMeasures[match.inventoryIngredient.name] !== undefined
                                  ? recipeMeasures[match.inventoryIngredient.name]
                                  : originalMeasure;
                              const isEdited = recipeMeasures[match.inventoryIngredient.name] !== undefined &&
                                recipeMeasures[match.inventoryIngredient.name] !== originalMeasure;
                              ...
                              return (
                                <ListItem
                                  ...
                                  sx={{
                                    ...
                                    borderRadius: 1,
                                    mb: 1,
                                    bgcolor: isEdited ? "action.selected" : "background.default",
                                  }}
                                >
                                  <Box
                                    sx={{
                                  ...
                                  >
                                    <Box sx={{ flex: 1, minWidth: 0 }}>
                                      <Typography variant="body1" fontWeight={500}>
                                        {translateIngredient(match.inventoryIngredient.name)}
                                      </Typography>
                                      <Typography variant="caption" color="text.secondary">
                                        {t("cooking.requiredInRecipe") || "Requerido en receta"}:{" "}
                                        {translateIngredient(match.recipeIngredientName)} ({match.recipeIngredientMeasure})
                                      </Typography>
                                    </Box>
                                    <Box
                                      sx={{
                                      ...
                                      <TextField
                                        ...
                                        onChange={(e) =>
                                          handleInventoryMeasureChange(
                                            recipeList.recipeName,
                                            match.inventoryIngredient.name,
                                            e.target.value
                                          )
                                        }
                                        placeholder={originalMeasure || t("cooking.noMeasure") || "Sin medida"}
                                      ...
                                      {isEdited && (
                                        <>
                                          <Button
                                            ...
                                            title={t("cooking.confirmInventory") || "Confirmar"}
                                          >
                                            ✓
                                          </Button>
                                          <Button
                      ...
                      <Box sx={{ display: "flex", justifyContent: "center", mt: 3 }}>
                        <Button
                          variant="contained"
                          size="large"
                          onClick={() => handleCompleteRecipe(recipeList.recipeName)}
                          sx={{
                            bgcolor: "#ff6b35",
                            color: "white",
                            fontSize: "1.2rem",
